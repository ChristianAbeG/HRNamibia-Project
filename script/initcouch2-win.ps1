...
$couch2="http://127.0.0.1:5986"

#YOU MUST MANUALLY ADD couchadmin/test as UN/PW for an admin account in Fauxton!
...
Invoke-RestMethod -Uri "$couch2/_config/http/authentication_handlers" -Headers @{Authorization=("Basic {0}" -f $base64AuthInfo)} -Method PUT -Body $body3
Invoke-RestMethod -Uri "$couch2/_config/couch_httpd_oauth/use_users_db" -Headers @{Authorization=("Basic {0}" -f $base64AuthInfo)} -Method PUT -Body '"true"'
